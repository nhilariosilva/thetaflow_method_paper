...
keep_valid_patients <- which(!is.na(tcga_brca_data$time) &
                             ...
                             (tcga_brca_data$time > 1))
tcga_brca_data_study <- tcga_brca_data[ , keep_valid_patients ]

# Gets only the protein-coding genes
keep_valid_genes <- which(
  gene_metadata$gene_type == "protein_coding"
)
tcga_brca_data_study <- tcga_brca_data[ keep_valid_genes , keep_valid_patients ]

# Total of 1071 valid patients and 19962 genes
...
X_transposed_log <- t( log2(1 + dds_matrix) )
X_transposed_log_means <- apply(X_transposed_log, 2, mean)
X_transposed_log_scaled <- t( t(X_transposed) - X_transposed_means )

time <- dds$time
delta <- dds$delta

final_dataset_log <- data.frame(time = time, delta = delta, X_transposed_log)
write.csv(final_dataset_log, file = "tcga_brca_count_data_coxnnet.csv", row.names = FALSE)
...
X_transposed_means <- apply(X_transposed, 2, mean)
X_transposed_scaled <- t( t(X_transposed) - X_transposed_means )
time <- vsd$time
delta <- vsd$delta

final_dataset <- data.frame(time = time, delta = delta, X_transposed_scaled)
